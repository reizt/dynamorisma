...
const buildConditionsRecursive = <E extends EntConfig>(entName: string, entConfig: E, where: Where<E>, addEntNameFilter: boolean): Conditions => {
  ...
  if (addEntNameFilter) {
    conds.and = conds.and ?? [];
    conds.and.push({ condition: { name: entNameAttrName, operator: '=', value: marshallValue(entName) } });
  }

  if (where.AND != null) {
    conds.and = conds.and ?? [];
    for (const andWhere of where.AND) {
      conds.and.push(buildConditionsRecursive(entName, entConfig, andWhere, false));
    }
  }
  ...
  if (where.OR != null) {
    ...
    for (const orWhere of where.OR) {
      conds.or.push(buildConditionsRecursive(entName, entConfig, orWhere, false));
    }
  }

  if (where.NOT != null) {
    conds.not = buildConditionsRecursive(entName, entConfig, where.NOT, false);
  }
  ...
  for (const propName in entConfig) {
    ...
    if (filter.eq != null) {
      propConds.push({ name: attrName, operator: '=', value: marshallValue(filter.eq) });
    }
    if (filter.ne != null) {
      propConds.push({ name: attrName, operator: '<>', value: marshallValue(filter.ne) });
    }
    if (filter.in != null) {
      propConds.push({ name: attrName, operator: 'in', valueIn: marshallValue(filter.in) });
    }
    if (propConfig.type === 'S') {
      const filterS = filter as Filter<{ type: 'S' }>;
      if (filterS.beginsWith != null) {
        propConds.push({ name: attrName, operator: 'begins_with', value: marshallValue(filterS.beginsWith) });
      }
      if (filterS.contains != null) {
        propConds.push({ name: attrName, operator: 'contains', value: marshallValue(filterS.contains) });
      }
    }
    if (propConfig.type === 'N') {
      const filterN = filter as Filter<{ type: 'N' }>;
      if (filterN.gt != null) {
        propConds.push({ name: attrName, operator: '>', value: marshallValue(filterN.gt) });
      }
      if (filterN.gte != null) {
        propConds.push({ name: attrName, operator: '>=', value: marshallValue(filterN.gte) });
      }
      if (filterN.lt != null) {
        propConds.push({ name: attrName, operator: '<', value: marshallValue(filterN.lt) });
      }
      if (filterN.lte != null) {
        propConds.push({ name: attrName, operator: '<=', value: marshallValue(filterN.lte) });
      }
      if (filterN.between != null) {
        propConds.push({ name: attrName, operator: 'between', valueFrom: marshallValue(filterN.between[0]), valueTo: marshallValue(filterN.between[1]) });
      }
    }
  ...
  if (conds.and.length === 1) {
    return {
      condition: conds.and[0]!.condition!,
    };
  }

  if (conds.or?.length === 1) {
    return {
      condition: conds.or[0]!.condition!,
    };
  }
